...
# Compose config check (uses .env.prod)
& docker compose --env-file $ProdFile -f docker-compose.prod.yml config | Out-Null
if ($LASTEXITCODE -ne 0) {
  Write-Host "[FAIL] docker compose config failed."
  exit 1
}

Write-Host "[OK] docker compose config succeeded."
