...
이 문서는 ai 를 이용해 생성한  **사내망 내부 서버**에서 Docker Compose로 서비스를 배포/롤백할 때, 실패를 일찍 발견하고(Preflight/Healthcheck) 기록을 남기기(release.log) 위한 **런북(runbook)**입니다.
...
| 운영 절차 | `ops/deploy.md` | 배포/롤백/리허설 런북 | 레포 `ops/` |
| 운영 기록 | `ops/release.log` | 배포 결과 기록(성공/실패) | 서버에 생성(커밋 금지) |
| 기록 템플릿 | `ops/release.log.template` | `release.log` 작성용 템플릿 | 레포(커밋 가능) |
...
### 2.1 공통
- 이미지 태그 고정: `gpu_chatbot_test-backend:<TAG>`, `gpu_chatbot_test-frontend:<TAG>`, `db`는 `pgvector/pgvector:pg16` 고정
...
### 2.3 Backend (FastAPI)
코드 수정 없이 사용할 수 있는 확인 경로로 `/api/openapi.json`을 사용합니다.
```yaml
healthcheck:
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/preflight_all.ps1 -ProdFile .env.prod
```
...
간단 확인(1줄):
```bash
curl -fsS http://localhost/ >/dev/null && curl -fsS http://localhost/api/openapi.json >/dev/null
```

> 인증이 필요한 RAG 엔드포인트를 반드시 포함해야 하는 경우, 별도 토큰/권한 정책이 확정된 뒤에 추가합니다.

### 4.5 기록(release.log)
- 배포 성공/실패 결과와 <TAG>, 시간, 핵심 로그 위치를 `ops/release.log`에 남깁니다.
- 템플릿은 `ops/release.log.template`을 복사해 사용합니다.
...
- [ ] `<TAG>` 규칙이 결정되어 있고, `ops/release.log`에 기록할 준비가 되어 있다
...
- [ ] 실패 시 `ops/release.log`에 “Minimum Failure Record”를 남긴다
...
- 표준 형식(택1):
  ...
  - 커밋형: `git short SHA`
- 예시:
  - `gpu_chatbot_test-backend:2026.01.14`
  - `gpu_chatbot_test-frontend:2026.01.14`

---

## 9. 운영 1회 리허설(가능할 때)

리허설이 가능해지면 아래 순서로 1회 수행하고, 결과를 `ops/release.log`에 남깁니다.
